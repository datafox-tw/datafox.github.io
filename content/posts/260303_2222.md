...
當你有了自己的網域（例如 `datafox.tw`），如果想擁有一個專業的電子郵件（如 `contact@datafox.tw`），通常需要購買 Google Workspace 或 Microsoft 365，每個月都要噴掉至少 6 美金的訂閱費。
...
- `contact@datafox.tw` 👉 轉寄到我的 `gmail`
